...
object tmaxgraph extends JFXApp {
...

  {
    ...
    val plot1 = Plot.scatterPlot(lonsups, latsups, title = "Stations w/ increase in Max Temp", xLabel = "Longitude", 
        yLabel = "Latitude", symbolSize = 3, cg(variups))
    SwingRenderer(plot1, 600, 700)
  }
  {
    val cg = ColorGradient(0.0 -> BlueARGB)
    val plot2 = Plot.scatterPlot(lonsdowns, latsdowns, title = "Stations w/ decrease in Max Temp", xLabel = "Longitude", 
        yLabel = "Latitude", symbolSize = 3, cg(varidowns))
    SwingRenderer(plot2, 600, 700)
  }
